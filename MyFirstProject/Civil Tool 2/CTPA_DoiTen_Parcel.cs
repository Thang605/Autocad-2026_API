...
namespace Civil3DCsharp
{
    class CTPA_DoiTen_Parcel_Commands
    {
        ...
        /// <summary>
        /// Lệnh đổi tên nhanh Parcel - nhập trực tiếp tên mới
        /// </summary>
        [CommandMethod("CTPA_DoiTen_Parcel_Nhanh")]
        public static void CTPA_DoiTen_Parcel_Nhanh()
        {
            int processedCount = 0;

            A.Ed.WriteMessage("\nChọn từng Parcel để đổi tên nhanh (ESC để kết thúc)...");
            ...
            while (true)
            {
                ...
                // Get current parcel name first (read-only transaction)
                string currentName = "";
                using (Transaction trRead = A.Db.TransactionManager.StartTransaction())
                {
                    CivilParcel? parcelRead = trRead.GetObject(per.ObjectId, OpenMode.ForRead) as CivilParcel;
                    if (parcelRead != null)
                    {
                        currentName = parcelRead.Name;
                    }
                    trRead.Commit();
                }

                // Prompt for new name
                PromptStringOptions pso = new PromptStringOptions($"\nNhập tên mới cho Parcel [{currentName}]: ");
                pso.AllowSpaces = true;
                pso.DefaultValue = currentName;

                PromptResult pr = A.Ed.GetString(pso);
